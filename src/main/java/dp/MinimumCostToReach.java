...
public class MinimumCostToReach {
    ...
    public static int minCostDp(int[] cost) {
        ...
        int[] calCost = new int[cost.length];
        Arrays.fill(calCost, -1);
        minSol = Math.min(min(cost, 0, calCost), minSol);
        minSol = Math.min(min(cost, 1, calCost), minSol);

        return minSol;
    }


    public static int min(int[] cost, int i, int[] calCost) {


        int n = 0;
        if (i < cost.length) {
            if (calCost[i] == -1) {
                calCost[i] = Math.min(min(cost, i + 1, calCost), min(cost, i + 2, calCost)) + cost[i];
                n = calCost[i];
